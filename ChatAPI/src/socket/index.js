...
export function initializeSocket(httpServer) {
  const isWildcard = config.socketCors.origin === '*';
  const io = new Server(httpServer, {
    cors: {
      origin: isWildcard ? '*' : config.socketCors.origin,
      credentials: !isWildcard && config.socketCors.credentials,
  ...
  // Start periodic call cleanup job (every 30s — safety net for missed calls)
  startPeriodicCleanup(io, 30000);
  ...
  io.on(SOCKET_EVENTS.CONNECT, async (socket) => {
    ...
    // Handle join call room (for WebRTC signaling)
